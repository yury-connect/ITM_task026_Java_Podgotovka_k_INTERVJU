...
4. **Serializable**
    * ⏹️ Полная изоляция (_последовательное выполнение_)
...
_Чтение данных, которые еще не зафиксированы (не подтверждены транзакцией), и могут быть откатаны_


* **Неповторяемое чтение** (_Non-Repeatable Read_)  
_Данные, прочитанные повторно в одной транзакции, изменяются другой транзакцией между чтениями._


* **Фантомное чтение** (_Phantom Read_)   
_В повторной выборке появляются новые строки, добавленные другой транзакцией._


* **Потерянное обновление** (_Lost Update_)   
_Два процесса одновременно обновляют одну и ту же запись, и одно обновление затирает другое._
