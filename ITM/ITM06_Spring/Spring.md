...
| 🔧 Функциональность                                | 🟢 `BeanFactory`                                                                | 💚 `ApplicationContext`                                                                  |
|:---------------------------------------------------|:--------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------------|
| Загрузка бинов (_инициализация_)                   | 💤 Ленивое создание (`getBean()` _вызывает создание_)                           | ⚡ Агрессивная загрузка (_бин создаётся при старте контейнера, без вызова_ `getBean())`   |
| Регистрация  `BeanPostProcessor`                   | ❌ Ручная                                                                        | ✅ Автоматическая                                                                         |
| Регистрация  `BeanFactoryPostProcessor`            | ❌ Ручная                                                                        | ✅ Автоматическая                                                                         |
| Внедрение зависимостей по аннотациям               | ❌ Не поддерживается                                                             | ✅ Поддержка `@Autowired`, `@PreDestroy`                                                  |
| I18N (_интернационализация_)                       | ❌ Нет                                                                           | ✅ Поддерживается через `MessageSource`                                                   |
| Множественные конфигурационные файлы               | ❌ Нет (_один XML-файл_)                                                         | ✅ Да (можно указать массив XML-файлов)                                                   |
...
| Публикация событий (`ApplicationEvent`)            | ❌ Нет                                                                           | ✅ Да, можно публиковать события, слушатели обрабатывают                                  |
| Корпоративные сервисы                              | ❌ Отсутствуют                                                                   | ✅ Поддержка: JNDI, EJB, удалённое взаимодействие и прочее                                |
|                                                    |                                                                                 |                                                                                          |
...
_(только тогда, когда все условия **соответствуют**)_.
...
📦 Это важно для формирования `ApplicationContext`: Spring ищет классы, создаёт их экземпляры и регистрирует как бины.
...
| `lazyInit`           | 	Если true — бины создаются лениво.                                   |
