...
```mermaid
graph TD
    A[Start: SpringBootApplication] --> B[EnableAutoConfiguration]
    B --> C[Import AutoConfig Selector]
    C --> D[Read spring.factories]
    D --> E[Load AutoConfig Classes]
    E --> F[Check @Conditional]
    F --> G[Create Beans If Needed]
    G --> H[Build ApplicationContext]
