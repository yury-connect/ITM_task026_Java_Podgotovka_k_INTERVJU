# Что такое `TIMESTAMP`?

---
### ⏳ `TIMESTAMP` vs `DATETIME` в _SQL_

#### 📌 **DATETIME**  
✔ **Формат**: `YYYY-MM-DD HH:MM:SS` *(просто цифры)* / Хранит дату и время как строку   
✔ **Диапазон:** От `1000-01-01 00:00:00` до `9999-12-31 23:59:59`   
✔ **Часовой пояс**: Независим от временной зоны *(т.е. не учитывает ее)*  
✔ **Размер**: 8 байт *(MySQL8.0: Без долей секунды: 5 байт, С долями -до 8 байт)*   
#### 📌 **TIMESTAMP**  *(зависит от часового пояса)*
✔ **Формат**: Хранит количество секунд с **1 января 1970** (_Unix Epoch_)  
✔ **Диапазон:** От `1970-01-01 00:00:01` UTC до `2038-01-19 03:14:07` UTC.
✔ **Часовой пояс**: Зависит от часового пояса сервера // 
	сервер _думает_, что это время вашего часового пояса.  
	При выборке автоматически корректируется под текущую зону  
✔ **Размер**: 4 байта *(как и стандартный Integer)* // до 7 байт с долями   

> **PostgreSQL**:
> *В отличие от `MySQL`, где `TIMESTAMP` это 4 байта, а `DATETIME` это 8 байт (или строка), в `PostgreSQL` всё унифицировано.*
> - **Тип данных в памяти/на диске:** **8-байтовое целое число** (`int64` / `bigint`).- **Что это за число?** Это количество **микросекунд** от начала эпоки (2000-01-01 00:00:00 UTC). Да, не от 1970 года, как в Unix, а от 2000-го (это особенность PostgreSQL).
> - В поле **`TIMESTAMP`** лежит число микросекунд от 2000-01-01.
> - В поле **`TIMESTAMPTZ`** лежит ровно такое же число микросекунд *от 2000-01-01*, но оно всегда интерпретируется как *UTC*.
## 🧐 Ключевые отличия

| **Характеристика**            | **DATETIME**                       | **TIMESTAMP**                                  |
| ----------------------------- | ---------------------------------- | ---------------------------------------------- |
| Формат хранения               | `YYYY-MM-DD HH:MM:SS`              | Число (_количество секунд с 1970/ Unix-время_) |
| Зависимость от часового пояса | ❌ Нет                              | ✅ Да                                           |
| Размер (байты)                | 8                                  | 4                                              |
| Скорость                      | Чуть медленнее *(вычитание строк)* | Чуть быстрее *(внутри — числа)*                |
| Диапазон значений             | `1000-01-01 → 9999-12-31`          | `1970-01-01 → 2038-01-19`                      |
| Использование                 | Даты рождения, события             | Временные метки, логи                          |

### 🔥 Когда использовать?

[](https://github.com/yury-connect/ITM_task026_Java_Podgotovka_k_INTERVJU/blob/by_questions/ITM/ITM04_SQL/sql.md#-%D0%BA%D0%BE%D0%B3%D0%B4%D0%B0-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D1%82%D1%8C)

✅ **DATETIME** → для хранения постоянных дат (_даты рождения, события_).

> - Минимальное значение: `1000-01-01 00:00:0`;
> - Максимальное значение: `9999-12-31 23:59:59`;

✅ **TIMESTAMP** → если нужно учитывать часовые пояса (_временные метки, логи, обновления записей_).

> - Минимальное значение: `1970-01-01 00:00:01 UTC` (_Unix Epoch + 1 секунда_);
> - Максимальное значение: `2038-01-19 03:14:07 UTC` (_Проблема 2038 года ⏳_);

#### 📌 Ограничение `TIMESTAMP`:

[](https://github.com/yury-connect/ITM_task026_Java_Podgotovka_k_INTERVJU/blob/by_questions/ITM/ITM04_SQL/sql.md#-%D0%BE%D0%B3%D1%80%D0%B0%D0%BD%D0%B8%D1%87%D0%B5%D0%BD%D0%B8%D0%B5-timestamp)

**TIMESTAMP** ограничен **32-битным целым числом** (`2147483647` **секунд от Unix Epoch**), поэтому **не поддерживает даты после 2038 года** (_Проблема **2038** года_ ⏳).

---

### DATE, TIME, DATETIME и TIMESTAMP

[](https://github.com/yury-connect/ITM_task026_Java_Podgotovka_k_INTERVJU/blob/by_questions/ITM/ITM04_SQL/sql.md#date-time-datetime-%D0%B8-timestamp)

|Тип|Описание|Диапазон значений|Размер|
|---|---|---|---|
|**DATE**|Хранит значения даты в виде `ГГГГ-ММ-ДД`. _Например, 2022-12-05_|`1000-01-01 → 9999-12-31`|4 байта|
|**TIME**|Хранит значения времени в формате `ЧЧ:ММ:СС` (_или `ЧЧЧ:ММ:СС` при значениях > 24 часов_). _Напр|`1000-01-01 00:00:00 → 9999-12-31 23:59:59`|8 байт|
|**TIMESTAMP**|Хранит значение даты и времени в виде `ГГГГ-ММ-ДД ЧЧ:ММ:СС`, в Unix-формате (_секунды с 1970-01-01_). _Например, 2022-12-05 10:37:22_|`1970-01-01 00:00:01 UTC → 2038-01-19 03:14:07 UTC`|4 байта|

---

```
***** из методички *****
DATETIME предназначен для хранения целого числа: YYYYMMDDHHMMSS. 
    И это время не зависит от временной зоны настроенной на сервере. 
    Размер: 8 байт
    
TIMESTAMP хранит значение равное количеству секунд, прошедших 
    с полуночи 1 января 1970 года по усреднённому времени Гринвича. 
    Тогда была создана Unix. 
    При получении из базы отображается с учётом часового пояса. 
    Размер: 4 байта
```