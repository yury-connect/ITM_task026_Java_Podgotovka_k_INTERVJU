...
## **1. По умолчанию (*сохраняет `ordinal()` — числовой индекс*)**
```java
public enum Status {
...
## **4. Маппинг через отдельный столбец с `@PostLoad` и `@PrePersist`**
...
|Ручное управление|`@PostLoad`/`@PrePersist`|Любое|Для сложных случаев, но много кода|
