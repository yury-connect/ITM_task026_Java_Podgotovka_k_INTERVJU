...
[⬅️**Previous**](Группа_04-Управление_транзакциями)   //   [⬆️](SPRING-121_вопрос_на_middle)   //   [**Next**➡️](Группа_06-Spring_Boot_Core)
...
**Ответ:** `DispatcherServlet` — это **центральный сервлет-диспетчер** в Spring MVC, реализующий паттерн **Front Controller**. Он принимает все входящие HTTP-запросы (обычно по маппингу `/`) и направляет их в соответствующие обработчики (контроллеры).
