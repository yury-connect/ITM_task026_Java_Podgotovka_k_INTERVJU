# Жизненный цикл **бина** в **Spring Boot**
...
####  Spring сначала работает не с объектами, а с метаданными (BeanDefinition)
>👉 Создаются не все сразу: 
>- `singleton` → eager (по умолчанию)
>- `lazy` → при первом вызове
>- `prototype` → каждый раз новый
#### Где происходит “магия Spring”
>👉 3 ключевые точки:   
>1. `BeanDefinitionRegistryPostProcessor` → меняем структуру контекста
>2. `BeanFactoryPostProcessor` → меняем конфигурацию
>3. `BeanPostProcessor` → меняем сами объекты (прокси)
#### Где создаётся AOP
>✔️ На этапе `postProcessAfterInitialization`
#### Почему важен этап 0
> ✔️ Потому что без `ConfigurationClassPostProcessor` не будет:
>- `@Bean`
>- `@ComponentScan`
>- `@Import`

---
# 🧠 Как запомнить (улучшенная модель)
Не 7 шагов, а **3 уровня:**
###### 🟦 УРОВЕНЬ 1 — МЕТАДАННЫЕ
	`Scan` → `BeanDefinition` → `PostProcessors`
###### 🟩 УРОВЕНЬ 2 — СОЗДАНИЕ
	`Instantiate` → `Inject` → `Aware`
###### 🟨 УРОВЕНЬ 3 — ЖИЗНЬ
	`Init` → `Proxy` → `Work` → `Destroy`

## 💬 Идеальный ответ *(middle+ уровень)*
> Жизненный цикл бина в Spring начинается со сканирования классов и создания BeanDefinition — это метаданные о бине.  
> Далее BeanDefinition могут быть изменены через BeanDefinitionRegistryPostProcessor и BeanFactoryPostProcessor.  
> После этого создаётся экземпляр бина, внедряются зависимости, вызываются Aware-интерфейсы.  
> Затем бин проходит через BeanPostProcessor до и после инициализации.  
> В фазе инициализации вызываются @PostConstruct, afterPropertiesSet и initMethod.  
> После postProcessAfterInitialization бин может быть обёрнут в прокси (например для AOP).  
> После этого бин используется, а при закрытии контекста вызываются методы уничтожения.
...
└──────────────────────────────────────────────┘
        ...
        ▼
┌──────────────────────────────────────────────┐
│ 3. ДОБАВЛЕНИЕ / МОДИФИКАЦИЯ DEFINITION       │
│   - BeanDefinitionRegistryPostProcessor      │
│         → может ДОБАВЛЯТЬ BeanDefinition     │
│   - BeanFactoryPostProcessor                 │
│         → модифицирует BeanDefinition        │
└──────────────────────────────────────────────┘
        │
        ▼
┌──────────────────────────────────────────────┐
│ 4. ОБРАБОТКА POST PROCESSOR                  │
...
└──────────────────────────────────────────────┘
        ...
        ▼
┌──────────────────────────────────────────────┐
│ 5. СОЗДАНИЕ ЭКЗЕМПЛЯРА (INSTANTIATION)       │
│   - Вызов конструктора                       │
...
└──────────────────────────────────────────────┘
        ...
        ▼
┌──────────────────────────────────────────────┐
│ 6. ВНЕДРЕНИЕ ЗАВИСИМОСТЕЙ (POPULATION)       │
│   - @Autowired (constructor / setter / field)│
...
└──────────────────────────────────────────────┘
        ...
        ▼
┌──────────────────────────────────────────────┐
│ 8. ПРЕД-ИНИЦИАЛИЗАЦИЯ (BPP)                  │
│   - postProcessBeforeInitialization          │
│   - (@PostConstruct вызывается через BPP)    │
└──────────────────────────────────────────────┘
        │
        ▼
┌──────────────────────────────────────────────┐
...
└──────────────────────────────────────────────┘
        ...
        ▼
┌──────────────────────────────────────────────┐
│ 10. ПОСТ-ИНИЦИАЛИЗАЦИЯ (BPP)                 │
│   - postProcessAfterInitialization           │
│   - AOP / прокси (@Transactional и др.)      │
└──────────────────────────────────────────────┘
        │
        ▼
┌──────────────────────────────────────────────┐
...
│   - Живёт в контейнере                       │
│   - Prototype → возвращён и не управляется   │
└──────────────────────────────────────────────┘
        │
        ▼
┌──────────────────────────────────────────────┐
│ 12. УНИЧТОЖЕНИЕ (закрытие context)           │
│   1) @PreDestroy                             │
...
---
## 💻 Код, который показывает ВСЕ этапы 
(включая сканирование)
...
### 4. Кастомный `BeanPostProcessor` 
*(чтобы увидеть ЭТАПЫ 6 и 8)*
```java
@Component
...
---
## 🎯 Почему этапы 0–2 критически важны 

| Этап                            | Что это даёт                                   | Реальный пример                                                        |
| ------------------------------- | ---------------------------------------------- | ---------------------------------------------------------------------- |
| **0. Сканирование**             | Находит классы-кандидаты без твоего участия    | `@SpringBootApplication` <br>автоматически <br>сканирует текущий пакет |
| **1. BeanDefinition**           | Позволяет изменить метаданные ДО создания бина | Сделать бин `lazy`, <br>изменить scope, <br>добавить `@DependsOn`      |
| **2. BeanFactoryPostProcessor** | Модифицирует все определения сразу             | `PropertySourcesPlacehol<br>derConfigurer` заменяет `${...}`           |

### Пример мощи этапа 2 
*(код, который меняет игру)*
```java
@Component
