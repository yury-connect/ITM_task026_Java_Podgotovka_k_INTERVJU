# **Уровни изоляции по умолчанию** в популярных СУБД
...
1. **PostgreSQL**    
    ...
    - Для устранения **фантомов** на уровне `REPEATABLE READ` использует [**MVCC**](=PostgreSQL=/MVCC/=MVCC(1).md)  (многоверсионность) 4.
    
2. **MySQL (InnoDB)**    
    - На уровне `REPEATABLE READ` также предотвращает фантомы благодаря [**MVCC**](=PostgreSQL=/MVCC/=MVCC(1).md)  13.        
...
- **`REPEATABLE READ`** (по умолчанию в MySQL/MariaDB):    
    - Для задач, где важно повторяемое чтение (*например, финанс. операции*) 13.
