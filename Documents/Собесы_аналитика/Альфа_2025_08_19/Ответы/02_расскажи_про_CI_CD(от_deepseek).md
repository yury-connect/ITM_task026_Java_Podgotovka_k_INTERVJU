...
- **Ветка `test` (или `release/*`)**:    
    - **Назначение:** Ветка для стабилизации и тестирования готового к выпуску функционала.        
    ...
    - **Что в нее попадало:** _Только_ горячие фиксы (hotfixes) и исправления багов, найденных QA на тестовом стенде. Новая функциональность в `test` уже не добавлялась.        
...
3. Запускается **PR-билд** в Jenkins: проверка стиля, сборка, unit-тесты. Это _pre-merge_ проверка.    
